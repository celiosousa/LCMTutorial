...
Cultivated land in Liberia primarily consists of woody commodity crops (oil palm and rubber trees). The spectra of these woody crops are identical with those of natural vegetation. However, these areas generally have regular distribution patterns such as square or rectangles. The availability of forest concessions datasets is of paramount importance to constrain the classification of woody crop areas within the area they are likely to be found and avoid misclassification with other vegetation classes, such as evergreen forest.

Here, we are going to constrain the classification within areas of Rubber and Oil Palm Concessions. The data was previously uploaded into GEE and you can have access to it by loading it into your script with `ee.FeatureCollection('users/capacityBuilding/Liberia/Rubber')` and `ee.FeatureCollection('users/capacityBuilding/Liberia/OilPalm')`.

We will import these concessions along with the rest of the assets:
...
Map.addLayer(composite, {bands: ['SR_B5', 'SR_B6', 'SR_B4'], min:0.1, max:0.2}, 'Composite');
Map.addLayer(rubber, {color:'red'});
Map.addLayer(oilpalm, {color:'green'});
