...
This section demonstrates how to calculate and add spectral indices to each scene of a Landsat 8 image collection. The function below calculates and add spectral indices to an image collection taking into account the Landsat 8 spectral bands (Table 2.1):

```{r my-fig,  fig.cap="Landsat 8 bands", echo=FALSE, message=FALSE}
knitr::include_graphics("images/TableBandsL8.PNG")
```

The function to calculate several spectral indices follows this format:
```{r eval=FALSE}
