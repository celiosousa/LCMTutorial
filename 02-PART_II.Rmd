...
In this section we will provide functions and lines of code to assist the user in preparing, processing, and analyzing Landsat 8 surface reflectance data for land cover classification purposes.

### Cloud and cloud shadow masking

This sub-section demonstrates one way of masking clouds and cloud shadow pixels from Landsat 8 Surface Reflectance Collection 2 data based on on metadata within the file. You may notice when exploring Landsat images in the __Console__ tab that these images have a band called `QA_PIXEL` or *Quality Assessment* band. Briefly, this band contains values that represent bit-packed combinations of surface, atmospheric, and sensor conditions that can affect the overall usefulness of a given pixel. One of the many bits represented in this band is *cloud* (bit 3) and *cloud shadow* (bit 4):
...
You don’t need to worry about the new methods in this function. What you need to know is that this function was constructed in a way that we are selecting the `QA_PIXEL` band from the Landsat image and creating a mask where only pixels flagged as 0 (indicating clear conditions) for bits 3 and 4 will be included; The function will return `image` - in this case the Landsat 8 image - masked for pixels that are not flagged as 0 for bits 3 and 4. The `.divide()` was used simply to scale the band values to [0,1] and `.select()` to only select the spectral bands from Landsat (SR_B1-9). These steps are not necessary for the cloud masking to work! However, we can add these steps to further refine our image collection.
...
You can then classify unlabeled samples using the trained classifier. The final classification of a sample is based upon the majority of the votes of the trees in the ensemble:
...
For this example, let us classify the 'composite' into four classes: *water*, *agricultural land*, *sand and bare areas* and *urbanization*. The first step is to create the training samples set to use into the Random Forest Model.
...
Repeat the process for each of the other classes. Make sure you select samples that are representative of each land cover class by selecting points and polygons of homogeneous pixels.
```
...
Using the split value above, roughly 80% of the features in `samples` will be `training` and 20% will be in `testing`:
...
As explained earlier, the trained classifier can be used to classify the `compositeMedian`. Google Earth Engine will run each pixel through each of the 200 trees in the classifier and assing it the label which received the majority of votes among all the trees:
...
* `maxPixels`: This is somewhat important argument. In order to avoid memory errors, the Earth Engine default is 1x10e8 pixels. If you do not define your max pixels and your exports exceeds the default, you will get an error.  
...
We can add the samples to the map using `Map.addLayer()`. To colorize geometries, you use  the `color` parameter instead of `palette`:
...
A new version of the `Cairo2022` map where *__Sand and bare areas (pixel value = 2)__* and *__Built-up and Urban Areas (pixel value = 3)__* are merged into a new class called *__Barren land and Artificial Surfaces (pixel value = 2)__*. Note that this new class can assume any value when merging, as long as both classes have the same value: 
...
This scenario is one of the most commonly used post-classification procedure where the goal is to remap specific areas to fix for classification errors. To illustrate, we will consider the following example:
...
In your own version of `Cairo2022` map, create a new geometry over an area of your interest and name it `aoi`: 
...
Then, we reapply the `.focal_median()` filter with `.reproject(projection.atScale(scale))`:
...
Access the full script for this section [here.](https://code.earthengine.google.com/c1151eecd2e21f2ae43aa37cf1fd80d2){target="_blank"}
